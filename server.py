...
def allowed_file(filename):
    allowed_extensions = set(
        ['png', 'tga', 'jpg', 'jpeg', 'gif', 'tif', 'tiff'])
    return ('.' in filename and
            filename.rsplit('.', 1)[1].lower() in allowed_extensions)


def is_data_file(filename):
    return "data" in request.form or (
        '.' in filename and filename.rsplit('.', 1)[1].lower() == "data")
        ...
        try:
            ...
            if not obj_file or '.' in obj_file.filename and (
                ...
                return render_template(
                    "min-error.html", error_msg="No OBJ model provided.")

            tex_file = upload_file("tex-input")
            if not tex_file:
                return render_template(
                    "min-error.html", error_msg="No texture image provided.")
            ...
            if(is_data_file(tex_file.filename)):
                ...
                isFloatTexture = not issubclass(
                    textureData.dtype.type, numpy.integer)
                if(not isFloatTexture):
                    textureData = textureData / 255.0
            height, width, depth = (textureData.shape + (1,))[:3]

            sv_methods = {
                "none": seam_erasure.SeamValueMethod.NONE,
            ...
            if isDataFile:
                ...
                if(type(data_uri) is bytes):
                    data_uri = data_uri.decode()
                try:
                    return render_template(
                        'min-results.html', min_tex=data_uri,
            ...
            return render_template('min-error.html', error_msg=(
                "Unable to erase the texture ({}).".format(e)))
    return render_template('min-form.html')
