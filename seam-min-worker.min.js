...
FindSeam.find_seam=function(a){for(var c=a.f.map(function(a){return a.map(function(a){return a.v})}),b=a.f.map(function(a){return a.map(function(a){return a.vt})}),d={},e=0;e<c.length;e++){var f=c[e];d[[f[0],f[1]]]=[e,[0,1]];d[[f[1],f[2]]]=[e,[1,2]];d[[f[2],f[0]]]=[e,[2,0]]}f=Object.keys(d);c=new Set;for(e=0;e<f.length;e++){var g=JSON.parse("["+f[e]+"]");c.add(""+[Math.min(g[0],g[1]),Math.max(g[0],g[1])])}for(var c=Array.from(c).map(function(a){return JSON.parse("["+a+"]")}),f=[],g=[],l=[],e=0;e<
...
function range_min_max(a,c){return numeric.range(Math.floor(Math.min(a,c)),Math.floor(Math.max(a,c)))}function round(a,c){return Number(Math.round(a+"e"+c)+"e-"+c)}function print_progress(a){a="\r"+round(100*a,2)+"%";log_output(a)}
function log_output(a,c){if(c)console.log(a);else if(USING_WORKER)self.postMessage({id:"output",out_str:a});else{var b=document.getElementById("output");null!==b&&(""===b.innerHTML&&(b.innerHTML="<h3>Output:</h3>"),b.innerHTML+=a.replace("\n","<br>")+"<br>");console.log(a)}}function is_counterclockwise(a,c,b){return 0<numeric.det([[1,a[0],a[1]],[1,c[0],c[1]],[1,b[0],b[1]]])}function to_uint8(a){a=numeric.saturate(a);return a=numeric.round(numeric.mul(a,255))};var USING_WORKER=!0;function minimize(a,c){var b=OBJReader.quads_to_triangles(OBJReader.parse_obj(c.split("\n"),a)),d=numeric.ones([100,100,3]),b=SeamMinimizer.solve_seam(b,d);console.log(numeric.reshape(b,[100,100,3]))}self.onmessage=function(a){"input"===a.data.id?minimize(a.data.fname,a.data.objFile):console.log(a)};