...
def compute_edge_intervals(edge, width, height):
    ...
    interval |= set(x_to_t(x) for x in
    range_min_max(math.ceil(v0.x), math.ceil(v1.x)))

    interval |= set(y_to_t(y) for y in
    range_min_max(math.ceil(v0.y), math.ceil(v1.y)))
...
def compute_seam_intervals(uv_seam, width, height):
    ...
    return [compute_edgePair_intervals(edgePair, width, height)
