...
def bilerp_coeffMats(uv0, uv1, p00, p10, p01, p11, nPixels, debug = False):
...
def E_ab(a, b, edgePair, width, height, debug = False):
    """
    ...
    (A, B, C)  = bilerp_coeffMats(luv0, luv1, 0, 1, 2, 3, 8, debug = debug)
    (Ap, Bp, Cp) = bilerp_coeffMats(luv0p, luv1p, 4, 5, 6, 7, 8, debug = debug)
...
def E_edgePair(edgePair, width, height, edge_len, debug = False):
    ...
    for a, b in pairwise(intervals):
        ...
        # do the +.
        # E_edge += E_ab(a, b, edgePair, width, height, debug = debug)

        # Grab the guts of each coo matrix.
        E_edge.add(E_ab(a, b, edgePair, width, height, debug = debug))
...
def E_total(seam, width, height, depth, edge_lens, debug = False):
    """ Calculate the energy coeff matrix for a width x height texture. """
    print("Building Bilinear Energy Matrix:")
    ...
    for i, (edgePair, edge_len) in enumerate(zip(seam, edge_lens)):
        ...
        E.add(E_edgePair(edgePair, width, height, edge_len, debug))
